...
| POST | `/stream/{session_id}` | **Optional:** Receive streaming chunks |
| POST | `/stream/{session_id}/complete` | **Optional:** Mark session complete |
...
- `wait-for-session=<timeout>` - Wait for session creation if it doesn't exist yet, with specified timeout (e.g., `wait-for-session=30s`)
...
Receives real-time chunks from the ARK controller for streaming to connected clients. This endpoint accepts newline-delimited JSON chunks in OpenAI streaming format.
...
**POST** `/stream/{session_id}/complete`

Signals that query execution is complete and marks the streaming session as finished. This endpoint is called by the ARK controller when query execution finishes to notify streaming clients that no more chunks will be sent.
...
**Example Usage:**
```bash
curl -X POST "http://memory-service:8080/stream/my-session/complete"
```
