...
Establishes a Server-Sent Events (SSE) connection to stream messages in real-time during query execution.

**Query Parameters:**
- `from-beginning=true` - Send all existing messages first, then stream new ones
- `wait-for-query=<timeout>` - Wait for query execution to start if it doesn't exist yet, with specified timeout (e.g., `wait-for-query=30s`)
...
**Example Usage:**
```bash
curl -N "http://memory-service:8080/stream/my-session?from-beginning=true&wait-for-session=30s"
```
...
Receives real-time chunks from the ARK controller during query execution for streaming to connected clients. This endpoint accepts newline-delimited JSON chunks in OpenAI streaming format.

**Content-Type:** `application/x-ndjson`

**Request Body:** Stream of OpenAI-compatible chunks:
```json
{"id": "session-uuid", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Hello"}}]}
{"id": "session-uuid", "object": "chat.completion.chunk", "choices": [{"delta": {"content": " world"}}]}
{"id": "session-uuid", "object": "chat.completion.chunk", "choices": [{"finish_reason": "stop"}]}
```

**Example Usage:**
```bash
curl -X POST "http://memory-service:8080/stream/my-session" \
  -H "Content-Type: application/x-ndjson" \
  --data-raw '{"id": "my-session", "object": "chat.completion.chunk", "choices": [{"delta": {"content": "Hello world"}}]}'
```

### Complete Stream

**POST** `/stream/{query_id}/complete`

Signals that query execution is complete and marks the streaming context as finished. This endpoint is called by the ARK controller when query execution finishes to notify streaming clients that no more chunks will be sent.

**Why this is needed:** In ARK's distributed architecture, the controller orchestrates execution across multiple agents/teams/models, so it needs to explicitly signal completion to the memory service. Unlike direct LLM APIs where the model sends completion as part of the stream, ARK's controller manages the execution lifecycle separately.

**Example Usage:**
```bash
curl -X POST "http://memory-service:8080/stream/my-query-id/complete"
```
...
When a stream is marked complete, all connected streaming clients receive a final chunk with `finish_reason: "stop"` followed by the termination marker `data: [DONE]`.
