...
A dedicated event-stream resource will later be created. At this stage, a `Memory` resource which implements the [Event Stream API](#event-stream-api) can be used. Set the `ark.mckinsey.com/memory-event-stream-enabled` annotation to indicate that a memory supports streaming. The `ark-cluster-memory` service implements this API.
...
The Ark OpenAI completions API can return a streaming response:
...
The response will be Server-Sent Events (SSE) with streaming chunks as shown in the [Read Stream](#read-stream) API format, which is fully OpenAI compatible. This means that consumers can query Ark and stream responses as if it was an LLM, and use existing OpenAI SDKs etc.
...
sequenceDiagram
    ...
    Client->>Query Controller: Create Query resource
    Query Controller->>Streaming Service: POST /stream/{query_id} (chunks)
    ARK API->>Streaming Service: GET /stream/{query_id} (SSE)
    Client->>ARK API: GET /namespaces/{ns}/queries/{name}/stream (SSE)
    Query Controller->>Streaming Service: POST /stream/{query_id}/complete
```
...
If a query target does not support streaming (as is currently the case for some models), then as-per the OpenAI specification the streaming APIs will return the entire response in a single chunk at the end of query execution. OpenAI and Azure OpenAI models currently support stream, Amazon Bedrock models do not yet have streaming implemented. Streaming for agents and teams is not yet supported.
...
- [ ] feat: send full response as a single chunk when downstream streaming not enabled
- [ ] feat/docs: spec for metadata in stream chunks and target metadata in stream responses as well as query id and session id
...
- [ ] feat: bedrock and check other model types
...
```
ark/internal/genai/memory_http.go  - This is the main file with streaming implementation
ark/internal/genai/model_generic.go - This is where chunks are forwarded from the LLM
```
