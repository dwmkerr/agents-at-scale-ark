...
In most cases the default installation options will be sufficient. This will install the Ark dependencies, the controller, the APIs and the dashboard. You will need a Kubernetes cluster to install Ark into, you can use [Minikube](), [Kind](), [Kubernetes in Docker]() or similar to run a local cluster. The `install` command will warn if any required dependencies are missing.
